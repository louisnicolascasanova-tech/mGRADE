...
def parse():
    ...
    parser.add_argument("--model_code", default='alllayer_gate', type=str, required=False,
                        help="String that denotes a unique type of training. Default options are alllayer{_gate/candidate}, firstlayer{_gate/candidate}, nolayer.")
    ...
    parser.add_argument("--decay_gate", default="11", type=str, required=False, 
                        help="Boolean string indicating whether decay dynamics on gate preactivation is enabled.")
    parser.add_argument("--train_gate_decay", default=False, type=bool, required=False, 
                        help="Boolean indicating whether decay time constants on gate preactivation are trained or fixed.")
    parser.add_argument("--decay_candidate", default="00", type=str, required=False, 
                        help="Boolean string indicating whether decay dynamics on h_tilde preactivation is enabled.")
    parser.add_argument("--train_candidate_decay", default=False, type=bool, required=False, 
                        help="Boolean indicating whether decay time constants on candidate preactivation are trained or fixed.")
    ...
    parser.add_argument("--reg_factor", default=0.0, type=float, required=False, 
                        help="Float representing the regularization factor.")
    
    parser.add_argument("--pre_mixing", default=False, type=bool, required=False, 
                        help="Add mixing between hidden states before hidden activation.")
    parser.add_argument("--hidden_activation", default="scaled_sigmoid", type=str, required=False, 
    ...
    parser.add_argument("--hidden_nonlinearity", default=None, type=str, required=False, 
                        help="IGNORE Helper argument IGNORE.")
    ...
    parser.add_argument("--round_weights", default=False, type=bool, required=False,
                        help="Whether to round weights to the nearest 0.5")
    ...
    if args.model_code == 'nolayer':
    ...
    elif args.model_code == 'firstlayer_gate_trainable':
        args.decay_gate = [False]*args.n_layers
        args.decay_gate[0] = True
        args.train_gate_decay = True
        args.decay_candidate = [False]*args.n_layers
        args.reg_factor = 0.0
    elif args.model_code == 'firstlayer_candidate_trainable':
        args.decay_gate = [False]*args.n_layers
        args.decay_candidate = [True]*args.n_layers
        args.decay_candidate[0] = True
        args.train_candidate_decay = True
        args.reg_factor = 0.0
    elif args.model_code == 'firstlayer_trainable':
        args.decay_gate = [False]*args.n_layers
        args.decay_gate[0] = True
        args.train_gate_decay = True
        args.decay_candidate = [False]*args.n_layers
        ...
        args.train_candidate_decay = True
        args.reg_factor = 0.0
    elif args.model_code == 'alllayer_gate_trainable':
        args.decay_gate = [True]*args.n_layers
        args.decay_candidate = [False]*args.n_layers
        args.train_gate_decay = True
    elif args.model_code == 'alllayer_candidate_trainable':
        args.decay_gate = [False]*args.n_layers
        args.decay_candidate = [True]*args.n_layers
        args.train_candidate_decay = True
    elif args.model_code == 'alllayer_trainable':
        args.decay_gate = [True]*args.n_layers
        args.decay_candidate = [True]*args.n_layers
        args.train_gate_decay = True
        args.train_candidate_decay = True
    ...
    print(f"Train Gate Decay: {args.train_gate_decay}")
    print(f"Decay Candidate: {args.decay_candidate}")
    print(f"Train Candidate Decay: {args.train_candidate_decay}")
    ...
    print(f"Pre Mixing: {args.pre_mixing}")
...
def main():
    ...
    if args.dataset=="cifar10":
        trainloader, val_loader, testloader, N_CLASSES, SEQ_LENGTH, IN_DIM = create_cifar_gs_classification_dataset(bsz=args.batch_size)
    else:
        trainloader, val_loader, testloader, N_CLASSES, SEQ_LENGTH, IN_DIM = create_mnist_classification_dataset(bsz=args.batch_size, root="/home/christian/LearningJAX/data/MNIST",  version=args.dataset_version)
    ...
    id_sim = f"mingru_heinsen/{args.dataset}/{args.dataset_version}_h{args.hidden_dim}_l{args.n_layers}_dyn_{args.model_code}_lr{args.lr}_act_{args.hidden_activation}{args.sigmoid_scale}{args.id_note}"
    ...
    state = create_train_state(key, BatchRNN, args.dataset_version, args)

    # initialise parameters
    params = model.init(key, jnp.zeros_like(batch_x[:args.batch_size]))
    params_init = params
    ...
    # save test accuracy
    print("Test accuracy:", test_acc)    # save results


    save_results(RESULT_IMM, args.model_code, args.seed, test_loss, test_acc, train_losses[-1], train_accuracies[-1], args.n_layers, n_params, args.hidden_dim, args.sigmoid_scale ,args.dataset, args.sweep_steepness)
